<?php 
	...
	function thong_bao ($id) {
		...
		if (isset($_POST['add_trademark'])) {
			...
			$sql = "UPDATE thong_bao SET name = '$name', note = '$note', user_id = '$user_id' WHERE id = $id";
			$result = mysqli_query($conn_vn, $sql);
			if ($result) {
				echo '<script type="text/javascript">alert(\'Bạn đã sửa được một thông báo.\')</script>';
			} else {
	...
	$info = $action->getDetail('thong_bao', 'id', $_GET['id']);

	$list_user = $action->getList('admin', 'admin_role', '2', 'admin_id', 'asc', '', '', '');
?>
