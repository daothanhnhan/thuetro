		...
		<tr>
			...
			<td>
				...
				<div class="list-action">
					...
					<?php if ($up_free == 0) { ?>
					<!-- <a onclick="up_mat_fee(<?= $item['product_id'] ?>)"><img src="/images/up.png" alt="icon"><span style="color: red;">Up mất phí</span></a> -->
					<?php } else { ?>
					...
					<a onclick="dang_lai(<?= $item['product_id'] ?>)"><img src="/images/dang-lai.png" alt="icon">Đăng lại</a>
				</div>
			</td>
		</tr>
		<?php } ?>
...
<script>
	function dang_lai (product_id) {
		...
		const xhttp = new XMLHttpRequest();
		  ...
		  xhttp.open("GET", "/functions/ajax/dang_lai.php?product_id="+product_id, true);
		  xhttp.send();
	}
	...
	function up_mat_fee (product_id) {
		// alert(product_id);
		const xhttp = new XMLHttpRequest();
		  xhttp.onload = function() {
		    // document.getElementById("demo").innerHTML = this.responseText;
		  ...
		  xhttp.open("GET", "/functions/ajax/up_mat_fee.php?product_id="+product_id, true);
		  xhttp.send();
	}
</script>