...
class library{
	...
	/*------------- Hàm select list ----------------*/
	    ...
	    public function getList_nha_moi_gioi($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	        ...
	        if (!empty($loai_tin)) {
	        	...
	        	if ($loai_tin == 2) {
	        		$where .= " AND cho_thue = 1 ";
	        	}
	        }

	        $loai_bds = $_GET['loai-bds'];
	        if (!empty($loai_bds)) {
	        	$arr_id = $this->get_children_procat($loai_bds);//var_dump($arr_id);
	        	if (empty($arr_id)) {
	        		$where .= " AND loai_bds in ($loai_bds)";
	        	} else {
	        		$arr_id[] = $loai_bds;
	        		$arr_id_text = implode(",", $arr_id);
	        		$where .= " AND loai_bds in ($arr_id_text)";
	        	}
	        ...
	        $tinh = $_GET['tinh'];
	        if (!empty($tinh)) {
	        	$where .= " AND city_id = '$tinh'";
	        }

	        $quan = $_GET['quan'];
	        if (!empty($quan)) {
	        	$where .= " AND district_id = '$quan'";
	        }

	        $cond = "&loai-tin=$loai_tin&loai-bds=$loai_bds&tinh=$tinh&quan=$quan";//echo $cond;
	    ...
	    public function getList_tim_du_an($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	        ...
	        if (!empty($_GET['name'])) {
	        	...
	        	foreach ($name_arr as $q) {
	        		$text = vi_en1($q);
	        		$where .= " OR friendly_url LIKE '%$text%' ";
	        	}
	        	$where .= ") ";
	        }
	        ...
	        $cond = "&name=$ten&loai=$loai&tinh=$tinh&quan=$quan";
...
//            $results = $stmt->get_result();
	...
	/*--------------- Danh mục đệ quy ---------------*/
		...
		public function get_children_procat ($parent) {
			global $conn_vn;
			$sql = "SELECT * FROM productcat WHERE productcat_parent = '$parent'";
			$result = mysqli_query($conn_vn, $sql);
			$arr_id = array();
			while ($row = mysqli_fetch_assoc($result)) {
				$arr_id[] = $row['productcat_id'];
			}
