...
class action_menu extends library{
    ...
    public function showMenu_byMultiLevel_mainMenuTraiCam_lang($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {
            ...
            foreach ($cate_child as $key => $item) {
                ...
                if ($row[$this->nameColTitle_menuLanguage] == 'Nhà đất bán') {
                    echo '<a href="'.$url.'" class="link_main_menu_'.($level+1).'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->list_nha_dat_ban();
                } elseif ($row[$this->nameColTitle_menuLanguage] == 'Nhà đất cho thuê') {
                    echo '<a href="'.$url.'" class="link_main_menu_'.($level+1).'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->list_cho_thue_dat();
                } elseif ($row[$this->nameColTitle_menuLanguage] == 'Dự án') {
                    echo '<a href="'.$url.'" class="link_main_menu_'.($level+1).'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->loai_du_an();
                } elseif ($row[$this->nameColTitle_menuLanguage] == 'Trang chủ') {
                    echo '<a href="'.$url.'" class="link_main_menu_'.($level+1).'"><i class="fa fa-home"></i></a>';
                } else {
                    echo '<a href="'.$url.'" class="link_main_menu_'.($level+1).'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->showMenu_byMultiLevel_mainMenuTraiCam($list_menu,$item[$this->nameColID_menu],$lang, $level+1);
                }
            ...
            // echo '
            //     <li style="padding-left:170px"><img src="/images/lang/ptd_en.jpg" style="width:18px;cursor: pointer;" onclick="lang_en()"></li>
            //     <li><img src="/images/lang/ptd_vn.jpg" style="width:18px;cursor: pointer;" onclick="lang_vn()"></li>
            // ';
            echo '</ul>';        
        } 
    }
    ...
    public function list_nha_dat_ban () {
        global $conn_vn;
        global $action;

        $list_procat = $action->getList('productcat', '', '', 'productcat_id', 'asc', '', '', '');

        echo '<ul class="menu-specia">';
        foreach ($list_procat as $item) {
            echo '<li>';
            $sub = '';
            $chu_dam = 'text-bold';
            if ($item['productcat_id'] == 102 || $item['productcat_id'] == 103) {
                $sub = '&nbsp;&nbsp;&nbsp;';
                $chu_dam = '';
            }
            $img = '<img src="/images/'.$item['productcat_img'].'" alt="icon" class="icon-menu">';
            echo '<a class="'.$chu_dam.'" href="/index.php?page=tim-kiem&title=&loai-tin=1&loai-bds='.$item['productcat_id'].'&tinh=0&quan=0&dien-tich=0&muc-gia=0&huong=0">'.$sub.$img.$item['productcat_name'].'</a>';
            echo '</li>';
        }
        echo '</ul>';
    }

    public function list_cho_thue_dat () {
        global $conn_vn;
        global $action;
        ...
        foreach ($list_procat as $item) {
            ...
            $img = '<img src="/images/'.$item['productcat_img'].'" alt="icon" class="icon-menu">';
            echo '<a class="'.$chu_dam.'" href="/index.php?page=tim-kiem&title=&loai-tin=2&loai-bds='.$item['productcat_id'].'&tinh=0&quan=0&dien-tich=0&muc-gia=0&huong=0">'.$sub.$img.$item['productcat_name'].'</a>';
            echo '</li>';
        }
        echo '</ul>';
    }
    ...
    public function loai_du_an () {
        global $conn_vn;
        global $action;

        $loai_du_an = $action->getList('loai_du_an', '', '', 'id', 'asc', '', '', '');

        echo '<ul class="menu-loai-du-an">';
        foreach ($loai_du_an as $item) {
            echo '<li>';
            echo '<a class="" href="/index.php?page=tim-du-an&name=&loai='.$item['id'].'&tinh=0&quan=0">'.$item['name'].'</a>';
            echo '</li>';
        }
        echo '</ul>';
    }
